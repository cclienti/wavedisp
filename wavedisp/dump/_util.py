...
def lz4_decompress(src: bytes, uncompressed_size: int) -> bytes:
    ...
    Only the block format is implemented, which is what FST stores: no
    frame header, no checksum, and the uncompressed size known from the
    section header. Some fifty lines of pure Python keep the package
    dependency-free -- the alternative being python-lz4 for the sole
    benefit of hierarchy blocks, which are a few hundred kilobytes even
    on large designs.

    :param bytes src: compressed block.
    :param int uncompressed_size: expected size of the result.
    ...
    while pos < end:
        ...
        if length == 15:
            while True:
                if pos >= end:
                    raise DumpError("truncated lz4 literal length")
                byte = src[pos]
                pos += 1
                length += byte
                if byte != 255:
        ...
        # The last sequence of a block stops after its literals: there is
        # no match offset to read.
        if pos >= end:
            break
        ...
        if offset == 0 or offset > len(dst):
            raise DumpError("invalid lz4 match offset")

        length = token & 0x0F
        if length == 15:
            while True:
                if pos >= end:
                    raise DumpError("truncated lz4 match length")
                byte = src[pos]
                pos += 1
                length += byte
                if byte != 255:
        ...
        if offset >= length:
            ...
            # Overlapping match: the copy reads bytes it just wrote, so
            # it cannot be done with a single slice.
            for index in range(length):
                dst.append(dst[start + index])

    if len(dst) != uncompressed_size:
        raise DumpError(f"lz4 block decompressed to {len(dst)} bytes, {uncompressed_size} expected")
